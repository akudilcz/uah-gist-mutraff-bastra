...
def readSumoNet(net_file):
    ...
    for edge in l_edges:
        if not (edge.get("function")=="internal"):
            ...
            dict_edge_weightss[edge_id]=traveltime

    return dict_edge_weightss
...
def genRandMap(file_name):
    ...
    for edge in dict_edge_weightss.keys():
        weight=dict_edge_weightss[edge]
...
def generatePenEdges(edges_list):
    ...
    new_dict_edge_weightss={}
    for edge in new_pen_edges:
        # ALVARO: Patched on 02/06/19
        # if not new_dict_edge_weightss.has_key(edge):
        if not edge in new_dict_edge_weightss:
            weight=dict_edge_weightss[edge] * weight_factor + weight_add
            new_dict_edge_weightss[edge]=weight
    ...
    xml_file.write("\t<interval begin=\"" + config["begin"] + "\" end=\""
                       + config["end"] + "\" id=\"whatever\">\n")
    for edge in new_dict_edge_weightss.keys():
        weight=new_dict_edge_weightss[edge]
        xml_file.write("\t\t<edge id=\"" + edge
                           + "\" traveltime=\"" + str(weight) + "\"/>\n")
    xml_file.write("\t</interval>\n")
    xml_file.write("</meandata>\n")
...
def join(list_maps, map_file):
    ...
    for map in list_maps:
        ...
        for edge in l_edges:
            ...
            # ALVARO: Patched on 02/06/19
            # if not new_dict_edge_weightss.has_key(edge_id):
            if not edge_id in new_dict_edge_weightss:
                new_dict_edge_weightss[edge_id]=traveltime
            else:
                if config["join_option"]=="add":
                    new_dict_edge_weightss[edge_id]=new_dict_edge_weightss[edge_id]+traveltime
                elif config["join_option"]=="max":
                    if new_dict_edge_weightss[edge_id]< traveltime:
                        new_dict_edge_weightss[edge_id]=traveltime
                else:
        ...
        xml_file.write("</meandata>\n")
        xml_file.close()
        new_dict_edge_weightss.clear()
        index=index+1

# ----------------------------------------------------
# Generate a list of N penEdges XML files
# ----------------------------------------------------
def generateNPenEdges(num_maps, prefix):
    iter=0
    maps=[]
    while iter < num_maps:
        name_map=config["output_dir"] + name + ".rand." + str(iter+1) + ".xml"
...
if __name__ == '__main__':
    ...
    dict_edge_weightss=readSumoNet(config["net_file"])

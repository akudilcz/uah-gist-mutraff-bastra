...
class MuTazCalculator:
      ...
      try:
        if( elem.tag ):
          if ( elem.tag == 'node' ):
            ...
            self.nodes_dict[elem.attrib['id']] ={ 'long':elem.attrib['x'],
                                             'lat': elem.attrib['y'],
  ...
  # --------------------------------------------------------------------------
  def dumpEdgesCsv(self):
    disclaimer='# Generated by mutraff_edges_csv.py , by alvaro.paricio@uah.es'
    flag_save_file = False
    if 'out_csv_edges' in self.opts and self.opts['out_csv_edges']:
      flag_save_file = True
      outfile=self.opts['out_csv_edges']
      print("dumping to file: "+outfile )
    
    line = "edge_id,odmat_id,from_node,to_node,from_lat,from_long,to_lat,to_long,center_lat,center_long,name"
    if flag_save_file:
      outF = open(outfile, 'w')
      outF.write( disclaimer+'\n' )
      outF.write(line+"\n")
    ...
    # self.edges / columns=['id','from','to','prio','type','numLanes','speed'] )
    # self.nodes / columns=['id','x','y','type'] )
    for odmat_id in self.edges:
       # print( self.edges[odmat_id] )
       e_from = self.edges[odmat_id]['from']
       e_to   = self.edges[odmat_id]['to']
       e_id   = self.edges[odmat_id]['id']
       if self.edges[odmat_id]['name']:
         e_name = repr(self.edges[odmat_id]['name'])
       else:
         e_name=''
       n_from_lat = float(self.nodes_dict[e_from]['lat'])
       n_from_long = float(self.nodes_dict[e_from]['long'])
       n_to_lat = float(self.nodes_dict[e_to]['lat'])
       n_to_long = float(self.nodes_dict[e_to]['long'])
       n_lat  = n_from_lat +( n_to_lat - n_from_lat ) / 2
       n_long = n_from_long+( n_to_long - n_from_long ) / 2

       line = "{},{},{},{},{},{},{},{},{},{}\n".format( e_id, odmat_id, e_from, e_to, n_from_lat, n_from_long, n_to_lat, n_to_long, n_lat, n_long,e_name)
       if flag_save_file:
         outF.write( line+"\n" )
       else:
         print( line )
